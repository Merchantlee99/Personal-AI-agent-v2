...
def load_agent_mappings() -> tuple[set[str], dict[str, str]]:
    config_candidates = []
    ...
    aliases_source = config.get("aliases")
    aliases: dict[str, str] = {}
    if isinstance(aliases_source, dict):
        for raw_alias, raw_target in aliases_source.items():
...
CANONICAL_IDS, ALIASES = load_agent_mappings()
...
def normalize_agent_id(raw: str) -> RoutingResult:
    ...
    alias_hit = ALIASES.get(token)
    if alias_hit:
        return RoutingResult(agent_id=alias_hit, fallback_used=False)
...
def build_markdown(
    ...
    lines.extend(
        [
            ...
            "- aliases accepted: ace/owl/dolphin",
