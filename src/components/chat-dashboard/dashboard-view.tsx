...
function renderMiniIcon(id: AgentSwitchId) {
  switch (id) {
    ...
    case "aegis":
      return <polygon points="12,2 21,9 18,21 6,21 3,9" stroke="currentColor" strokeWidth="1.5" fill="none" />;
    default:
      return null;
  }
}
...
function getSwitchHint(id: AgentSwitchId) {
  if (id === "minerva") return "Orchestrator";
  if (id === "clio") return "Knowledge";
  if (id === "hermes") return "Search";
  return "Guard";
}
...
function formatCalls(remaining: number, limit: number) {
    ...
    <div className={rootClass}>
      ...
      <div className={styles.layoutGrid}>
        <aside className={styles.leftRail}>
          <div className={styles.leftRailTitle}>에이전트</div>
          <div className={styles.leftRailDivider} />
          <div className={styles.agentList}>
            {AGENT_SWITCHES.map((item) => {
              ...
              return (
                ...
                >
                  <span className={styles.agentButtonIconWrap}>
                    <svg viewBox="0 0 24 24" width="20" height="20">
                      {renderMiniIcon(item.id)}
                    </svg>
                  </span>
                  <span className={styles.agentButtonText}>
                    <span className={styles.agentButtonTitle}>{item.title}</span>
                    <span className={styles.agentButtonHint}>{getSwitchHint(item.id)}</span>
                  </span>
                </button>
              );
            })}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className={styles.orbShell}>
                <Orb
                  colors={[currentAgent.color.glow, currentAgent.color.secondary]}
        ...
        <aside className={styles.rightPanel}>
          <div className={styles.monitorTitle}>Monitoring</div>
          <div className={styles.monitorCards}>
            <div className={styles.monitorCard}>
              <span className={styles.monitorLabel}>API 잔여 호출</span>
              <span className={styles.monitorValue}>
                {runtimeMetrics
                  ? formatCalls(runtimeMetrics.llm.remaining, runtimeMetrics.llm.dailyLimit)
                  : "-"}
              </span>
            </div>
            <div className={styles.monitorCard}>
              <span className={styles.monitorLabel}>보안 이슈</span>
              <span className={styles.monitorValue}>
                {runtimeMetrics ? runtimeMetrics.security.securityIssues : "-"}
              </span>
            </div>
            <div className={styles.monitorCard}>
              <span className={styles.monitorLabel}>열린 이슈</span>
              <span className={styles.monitorValue}>{runtimeMetrics ? runtimeMetrics.security.openIssues : "-"}</span>
            </div>
            <div className={styles.monitorCard}>
              <span className={styles.monitorLabel}>승인 대기</span>
              <span className={styles.monitorValue}>
                {runtimeMetrics ? runtimeMetrics.orchestration.pendingApprovals : "-"}
              </span>
            </div>
          </div>
          ...
          <div className={styles.monitorFooter}>
            <span>Last Sync</span>
            <span>{formatLastSync(runtimeMetrics?.generatedAt ?? null)}</span>
          </div>
          {runtimeMetricsError ? <div className={styles.monitorError}>metrics: {runtimeMetricsError}</div> : null}
        </aside>
