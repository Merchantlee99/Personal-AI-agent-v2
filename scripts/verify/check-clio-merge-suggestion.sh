...
memory_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
for rel in (
    f"obsidian_vault/01-Knowledge/Clio Merge Draft {run_id}.md",
    f"obsidian_vault/01-Knowledge/Clio Merge Candidate A {run_id}.md",
    f"obsidian_vault/01-Knowledge/Clio Merge Candidate B {run_id}.md",
):
    path = shared_root / rel
    if path.exists():
        path.unlink()
print("[clio-merge] cleanup ok")
PY
